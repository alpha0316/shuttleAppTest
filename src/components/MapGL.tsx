...
function MapGL({
  ...
  const [latestCoord, setLatestCoord] = useState<{ lat: number; lng: number; heading?: number; speed?: number; ignition?: any; timestamp?: number } | null>(null);
...
`;
  ...
  function getNearestStop(userCoords, locations, radius = 50) {
  ...
  useEffect(() => {
    ...
    client.on('connect', (packet) => {
      ...
      client.subscribe('traccar/positions', (err, granted) => {
        if (err) {
